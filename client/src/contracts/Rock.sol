...
contract Rock is ERC1155, ERC2981, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => mapping(address => NFTCard))) public usersCardsNfts;
    ...
    function mintCard(uint256 _cardId, uint _indexRealEstateInCollection) private onlyOwner returns (uint256)
    {
        _tokenIds.increment();
        ...
        NFTCard storage defaultOwner = usersCardsNfts[_indexRealEstateInCollection][newTokenId][contractOwner]; 
    ...
    function getLastPairMaticUsd (uint256 _amount) public view returns (uint256) {
