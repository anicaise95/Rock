...
export default function MyNfts() {
     const {
          state: { contract, accounts, web3, owner },
     } = useEth();
     const [myNfts, setMyNfts] = useState([]);
     const [uri, setURI] = useState([]);
     const [realEstateCount, setRealEstateCount] = useState(0);
     const [displayNFTs, setDisplayNFTs] = useState(0);
     ...
     useEffect(() => {
          console.log("MyProfile useEffect : récupération des biens immobiliers");
                                   ...
                                   <div class="col-6">
                                        <>
                                             {myNfts[indexRealEstate].map((NFTcard, cardId) => {
                                                  return (
                                                       <>
                                                            {cardId >= 0 && NFTcard.quantity > 0 && (
                                                                 <div key={cardId} class="grid">
                                                                      {cardId === 0 && (
                                                                           <div class="col-3">
                                                                                <img src={imgCarteCottage} className="" />
                                                                                {NFTcard.price} € / unitaire
                                                                           </div>
                                                                      )}
                                                                      {cardId === 1 && (
                                                                           <div class="col-3">
                                                                                <img src={imgCarteVilla} className="imgNFT" />
                                                                           </div>
                                                                      )}
                                                                      {cardId === 2 && (
                                                                           <div class="col-3">
                                                                                <img src={imgCarteMansion} className="imgNFT" />
                                                                           </div>
                                                                      )}
                                                                      {cardId === 3 && (
                                                                           <div class="col-3">
                                                                                <img src={imgCarteHighRise} className="imgNFT" />
                                                                           </div>
                                                                      )}
                                                                      <div class="col-3">Gain perçus : </div>
                                                                      <div class="col-3">Autre info</div>
